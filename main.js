...
function drawCanvas() {
  ...
  // Calculate canvas rotation for parallax effect
  ...
  layer_list.forEach(function (layer) {
    ...
    // Set opacity
    context.globalAlpha = layer.opacity != null ? layer.opacity : 1;

    // Calculate scale to fit canvas while maintaining aspect ratio
    var scaleX = canvas.width / layer.image.width;
    var scaleY = canvas.height / layer.image.height;
    var scale = Math.min(scaleX, scaleY);

    // Calculate centered position on canvas
    var x = (canvas.width - layer.image.width * scale) / 2 + layer.position.x;
    var y = (canvas.height - layer.image.height * scale) / 2 + layer.position.y;
